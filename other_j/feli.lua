    ...
    calculate = function(self, card, context)
        ...
        if context.before then
            for i, playing_card in ipairs(G.play.cards) do
                if playing_card.seal and playing_card:get_seal() == card.ability.extra.seal then
                    timer.enabled = false
                    print("saved")
                    timer.saved_time = G.TIMERS.REAL
                    for i = 1, pseudorandom("bd_rockstarbonnie", 1, card.ability.extra.max_generation) do
...
SMODS.Joker {
    ...
    add_to_deck = function(self,card,from_debuff)
        ...
        G.consumeables:emplace(card, 'front', true)
        G.consumeables.config.card_limit = G.consumeables.config.card_limit + card.ability.extra.mod_slot
    end,
}
