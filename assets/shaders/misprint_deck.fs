// by stupid (and like 2 lines by foo)
// the only difference is that this confirms to the shape of the sprite
...
extern PRECISION vec2 misprint_deck;
...
{
    vec2 mem_texture_coords = texture_coords.xy; // foo line
    ...
    float shiftX = 0.3 * sprite_width + 0.0000000000001 * misprint_deck.x;
    ...
    pixel.a = Texel(texture, mem_texture_coords).a; // other foo line
...
vec4 dissolve_mask(vec4 final_pixel, vec2 texture_coords, vec2 uv)
{
    ...
    if (final_pixel.a > 0.01 && burn_colour_1.a > 0.01 && !shadow && res < adjusted_dissolve + 0.8*(0.5-abs(adjusted_dissolve-0.5)) && res > adjusted_dissolve) {
        if (!shadow && res < adjusted_dissolve + 0.5*(0.5-abs(adjusted_dissolve-0.5)) && res > adjusted_dissolve) {
            final_pixel.rgb = burn_colour_1.rgb;
        } else if (burn_colour_2.a > 0.01) {
            final_pixel.rgb = burn_colour_2.rgb;
        }
    }
