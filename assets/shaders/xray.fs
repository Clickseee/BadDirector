...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    vec2 uv_big = (uv - 0.5) * 0.7 + 0.5;

    float lum = dot(tex.rgb, vec3(0.499, 0.587, 0.114));
    lum = pow(lum, 0.9);

    float edge = (0.32, 0.48);
    edge = pow(edge, 1.4);

    float interior = pow(1.0 - lum, 1.3) * 1.35;

    vec3 base_col = vec3(0.05, 0.08, 0.22);
    vec3 edge_col = mix(vec3(0.6, 0.5, 1.0), burn_colour_1.rgb, burn_colour_1.a);

    vec3 col = base_col;
    col += edge_col * edge * 1.1;
    col += vec3(0.15, 0.2, 0.25) * interior;

    col = mix(col, vec3(0.0), shadow ? 1.0 : 0.0);

    vec4 out_col = vec4(col, tex.a * (1.85 + edge * 1.4)) * colour;

    vec2 _keep_shader = xray * 0.0;
